...
//         $s_time_key = $this->getArrayKey($s_time);
 ...
 $patient_details = $this->api_model->getPatientById($patient);
        $patientname = $this->api_model->getPatientById($patient)->name;
        $doctorname = $this->api_model->getDoctorById($doctor)->name;
        $data = array();
        $data = array(
        ...
        if (empty($id)) {
            ...
            $this->api_model->insertPayment($data_appointment);
            $inserted_id = $this->db->insert_id('payment');
            $data_update_payment_id_in_appointment = array('payment_id' => $inserted_id);
            $this->api_model->updateAppointment($appointment_id, $data_update_payment_id_in_appointment);
    ...
    function getLabReports()
    {
        ...
        $userId = $this->api_model->getPatientByIonUserId($userionId)->id;
    ...
    function getLabReportDetails()
    {
        ...
        $userId = $this->input->post('user_ion_id');
        // $group = $this->input->post('group');
        $data['labreport'] = $this->api_model->getLabById($labreportid);
        if (!empty($data['labreport']->id)) {

            $data['settings'] = $this->api_model->getSettings();
            $data['doctor'] = $this->api_model->getDoctorById($data['labreport']->doctor);
            $data['user'] = $this->api_model->getPatientById($data['labreport']->patient);
            $data['message'] = 'successful';
